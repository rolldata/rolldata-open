    ...
    view = {
      ...
      asyncNode: function (setting, node, isSilent, callback) {
        ...
        $.ajax({
          ...
          //huwei 添加自定义异步请求参数对象模式
          data: setting.async.nestedData? {data: (tmpParam ? JSON.stringify(tmpParam) :"")} : setting.async.contentType.indexOf('application/json') > -1 ? JSON.stringify(tmpParam) : tmpParam,
          ...
          success: function (msg) {
            ...
            try {
              if (!msg || msg.length == 0) {
                ...
                //huwei  添加自定义回调参数获取
                newNodes = setting.async.key ? msg[setting.async.key] : msg;
              }
            } catch (err) {
              newNodes = msg;
            }
    ...
    _data = {
      ...
      getTreeCheckedNodes: function (setting, nodes, checked, results) {
        ...
        for (var i = 0, l = nodes.length; i < l; i++) {
          ...
          var children = data.nodeChildren(setting, node);
          var nodeChecked = data.nodeChecked(setting, node);
          //huwei 添加自定义获取勾选不排除筛选选项
          if ((node.nocheck !== true || setting.view.includeNocheck) && node.chkDisabled !== true && nodeChecked == checked) {
            results.push(node);
            if (onlyOne) {
              break;
            }
...
//	Override method in core
  ...
  if (!!_getTreeCheckedNodes) {
    data.getTreeCheckedNodes = function (setting, nodes, checked, results) {
      if (!!nodes && nodes.length > 0) {
        //huwei 添加自定义获取勾选不排除筛选选项
        if (!!nodes[0] && !!nodes[0].getParentNode() && !!data.isHidden(setting, nodes[0].getParentNode()) && !setting.view.includeNocheck) {
          return [];
        }
      }
  ...
  if (!!_getTreeChangeCheckedNodes) {
    data.getTreeChangeCheckedNodes = function (setting, nodes, results) {
      if (!!nodes && nodes.length > 0) {
        if (!!nodes[0] && !!nodes[0].getParentNode() && !!data.isHidden(setting, nodes[0].getParentNode())) {
          return [];
        }
      }
