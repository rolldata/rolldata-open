...
layui.define(function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'laymypage', DISABLED = 'layui-disabled'
  ...
  Class.prototype.view = function(){
    ...
    ,views = {
      ...
      //每页条数
      ,limit: function(){
        return '<span class="layui-laypage-limit"><input type="text" min="1" value="' + config.limit + '" class="layui-input" id="layui-laypage-limit-input" name="limit"></span>';
      }()
      //每页条数
      ,limits: function(){
      ...
      ,skip: function(){
        return ['<span class="layui-laypage-skip">&#x5230;&#x7B2C;'
          ,'<input type="text" min="1" value="'+ config.curr +'" class="layui-input" id="layui-laypage-skip-input" name="skip" />'
  ...
  //跳页的回调
  Class.prototype.jump = function(elem, type, isskip){
    if(!elem) return;
    var that = this
    ,config = that.config
    ,childs = elem.children
    ,btn = elem[tag]('button')[0]
    ,skipInput = doc[id]('layui-laypage-skip-input')
    ,limitInput = doc[id]('layui-laypage-limit-input')
    ,select = elem[tag]('select')[0]
    ,skip = function(){
      if(type == "skip"){
        var curr = Number(skipInput.value.replace(/\s|\D/g, ''));
        if(curr){
          config.curr = curr;
          that.render();
        }
      }else{
        var limit = Number(limitInput.value.replace(/\s|\D/g, ''));
        if(limit){
          config.limit = limit;
          that.render();
        }
  ...
  Class.prototype.skip = function(elem){
    if(!elem) return;
    var that = this, input = doc[id]('layui-laypage-skip-input');
    if(!input) return;
    laypage.on(input, 'keyup', function(e){
      ...
      if(keyCode === 13){
        that.jump(elem, 'skip', true)
      }
    });
  };
  Class.prototype.limit = function(elem){
    if(!elem) return;
    var that = this, input = doc[id]('layui-laypage-limit-input');
    if(!input) return;
    laypage.on(input, 'keyup', function(e){
      var value = this.value
      ,keyCode = e.keyCode;
