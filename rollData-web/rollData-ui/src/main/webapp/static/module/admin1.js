...
layui.define(["layer", 'setter', 'element', 'wdUtil', 'eleTree', 'form', 'notice', 'contextMenu'], function(f) {
	...
	,m = {
		...
		loadMoreSide: function(item, scrollto){
			...
			var url = setter.basePath + 'functionController/queryUserAvailableByParam',
			ajaxData = {isAdmin: item.isAdmin},
			homeType = h("#homeType").val(),
			callback = function(data){
				if(data){
					m.renderRootSideMenu(data.obj.treeNodes);
        ...
        renderSideMenu: function(name, funcid,hrefType, nodes, navtype){
            ...
            if(nodes && nodes.length){
				...
				h("#" + sidemenu).html('<li class="layui-nav-item layui-nav-itemed" style="text-align: center; color: #ccc;"><a class="itemds">暂无数据</a></li>');
			}
							...
							if(name == '模型管理'){
								...
								switch (v.type){
									...
									case "7":
										...
										break;
									case "8":
										v.iconSkin = 'iconfont icon-ico_biaodan';
										v.hrefLink = setter.basePath + "mobileFormController/mobileFormDesignPage?formId="+v.rsid+"&navname="+name+"&nodeid="+v.id+"&relationName="+v.name+"&resourcePath="+v.rsPath+"&menuId="+funcid
										break;	
									case "10":
										v.iconSkin = 'iconfont icon-more-report';
        ...
        renderRootSideMenu: function(nodes){
            ...
            if(nodes && nodes.length){
        		setNodePid(nodes, {id: 0, level: 0, hrefType: "0"});
							 ...
							 || v.hrefLink == 'resourceController/queryResourceTreeByMenuId'){
								...
								if(name == '模型管理'){
									...
									switch (v.type){
										...
										case "8":
											v.iconSkin = 'iconfont icon-ico_biaodan';
											v.hrefLink = setter.basePath + "mobileFormController/mobileFormDesignPage?formId="+v.rsid+"&nodeid="+v.id
											break;				
									}
								}
							}
