...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.renderBodyTd = function (d, indent, index, $td, col) {
        ...
        if (index === options.tree.iconIndex) {
            ...
            icon += '<span class="ew-tree-pack" style="display:contents">';
            // 加箭头
            var haveChild = d[options.tree.haveChildName];
            if (haveChild === undefined) haveChild = d[options.tree.childName] && d[options.tree.childName].length > 0;
            ...
            content = '<span style="display: contents">' + content + '</span>';
    ...
    /** 表格溢出点击展开功能 */
    $(document).on('mouseenter', '.ew-tree-table .ew-tree-table-cell.single-line', function () {
        var $content = $(this).children('.ew-tree-table-cell-content');
        if ($content.prop('scrollWidth') > $content.outerWidth()) {
            var that = this;
            var xx = $(this).children('.ew-tree-table-cell-content').text()
            layer.tips(xx, that,{
                tips: [1, "#66686d"],
                time: 9999999
            });
        }
        var $cont_input = $(this).children('.ew-tree-table-cell-content').find('input');
        if($cont_input) {
            if($cont_input.prop('scrollWidth') > $cont_input.outerWidth()) {
                var that = this;
                var contents_input = $cont_input.val()
                layer.tips(contents_input, that,{
                    tips: [1, "#66686d"],
                    area: [$cont_input.outerWidth() + 'px', 'auto'],
                    time: 9999999
                });
            }
        }
    }).on('mouseleave', '.ew-tree-table .ew-tree-table-cell.single-line', function () {
