...
layui.config({
    ...
    var $ = layui.$
        ...
        ,table = {
            ...
            ,addRow:function (tableId, index, data) {
                ...
                if(options.isTree){//处理层级
                    ...
                    if(uo) {
                        ...
                        uo.children.splice(childId,0,data);
             ...
             * （如果是树状则删除自己和子节点）
             * @param tableId
             * @param sourceData     移动行当前数据
             * @param targetIndex     移动位置
             * @param targetData     移动到位置数据
             * @param moveType       移动方式 append insertAfter insertBefore
             */
            ,moveRow: function(tableId, sourceData, targetData, moveType){
                var that=table.getClass(tableId)
                    ,options=that.config
                    ,index = 0            //新增插入坐标
                    ,moveData = JSON.parse(JSON.stringify(sourceData))
                    ,dataList = JSON.parse(JSON.stringify(table.getDataList(tableId)))
                    ,sonList=[]
                    ,list;

                    sonList = options.isTree && moveData.children.length ?table.treeFindSonList(that.config.id,moveData).reverse():[moveData];
                    list = dataList.splice(moveData[that.config.indexName], sonList.length);
                    table.delRow(tableId, sourceData);
                    
                    index = table.getDataMap(tableId)[targetData.id][that.config.indexName];
                    
                    switch(moveType){
                        case 'append':
                            list[0][options[TREE_UPID]] = targetData.id;
                            index++;
                            break;
                        case 'insertBefore':
                            list[0][options[TREE_UPID]] = targetData[options[TREE_UPID]];
                            index--;
                            break;
                        case 'insertAfter':
                            list[0][options[TREE_UPID]] = targetData[options[TREE_UPID]];
                    ...
                    list.length &&  list.forEach(function(v, i){
                        table.addRow(tableId, index, v);
                        index++;
                    });
            }
            /**
    ...
    Class.prototype.config = {
        ...
        //树相关图表
        ,openIcon: ['layui-icon-triangle-r', 'layui-icon-triangle-d'] //展开折叠图标
        ,branch: ['layui-icon-layer', 'layui-icon-layer'] //父节点
        ,leaf: 'layui-icon-file' //叶节点
    ...
    Class.prototype.resetDatas=function(list) {
        ...
        if(options.isTree){//树状
            ...
            datas[1] = treeList=that.resetDataTreeList(list,rootList);
            table.setDataTreeList(that.config.id,treeList);//设置树结构到缓存
            datas[0] = tableList=that.resetDataTableList(treeList);
    ...
    Class.prototype.treeConvertShowName=function (o) {
        var that = this
            ,options = that.config;
        var isTreeNode=(o.children&&o.children.length>0 || o[options.isfolder]);
        var temhtml='<div style="float: left;height: 28px;line-height: 28px;padding-left: '+
            ...
            +function () {//图标或占位符
                var temTreeHtml='';
                var temTreeIsOpen=o[table.config.cols.isOpen]? options.openIcon[1]:options.openIcon[0];
                if(isTreeNode){//父节点
    ...
    Class.prototype.treeNodeOpen=function (o,isOpen) {
        ...
        if(o[table.config.cols.isOpen]){//打开状态
            dbClickI[0].className = 'layui-icon layui-tree-head ' + that.config.openIcon[1];
        }else{
            dbClickI[0].className = 'layui-icon layui-tree-head ' + that.config.openIcon[0];
        }
    };
    ...
    Class.prototype.treeIconRender=function (o,isHtml) {
        var that = this
            ...
            ,isTreeNode=(o.children&&o.children.length>0 || o[options.isfolder]);
        var temTreeHtml='';
        if(iconOpen){
            ...
            if(isTreeNode){//父节点
                if((o[table.config.cols.iconOpen]||o[table.config.cols.iconClose])){
                    ...
                    temTreeHtml='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-'+ (o[table.config.cols.isOpen] ? "branch" : "leaf") +' '+ (o[table.config.cols.isOpen]?that.config.branch[1]:that.config.branch[0])+'" '+iconOpen+'></i>';
                }
            }else{//叶子节点
                if(o[table.config.cols.icon]){
                    temTreeHtml=temf();
                }else{
                    temTreeHtml+='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-leaf '+that.config.leaf+'" '+iconOpen+'></i>';
                }
            }
            if(isHtml){
                return temTreeHtml;
    ...
    Class.prototype.renderTdCss = function(){
        ...
        csid = that.index+"_"+MOD_NAME+'_style';
    ...
    Class.prototype.events = function(){
        ...
        //行双击事件
        bodytr.unbind('contextmenu').on('contextmenu',function (e) {
            var index=$(this).attr("data-index");
            var list=table.getDataList(that.config.id);
            var o=list[index];
            typeof options.onContextmenuRow === 'function' && options.onContextmenuRow(index,o);
            return false
        });
        ...
        th.unbind('mouseup').on('mouseup', function(e){
            if(dict.resizeStart){
                dict = {};
                _BODY.css('cursor', '');
                that.scrollPatch();
        ...
        //拖拽中
        thead.unbind('mousemove').on('mousemove', function(e){
            if(dict.resizeStart){
                e.preventDefault();
                if(dict.rule){
        ...
        //排序
        thead.unbind('click').on('click', function(){//点击标题列
            var othis = $(this)
        ...
        // 节点被拖拽的回调事件
        options.draggable && bodytr.unbind('mousedown').on("mousedown", function(e) {
            var time = 0;
            var eleNode = $(this);
            var index=parseInt(eleNode.attr("data-index"));
            var list=table.getDataList(that.config.id);
            var o=list[index];
            var isStopDrag = false;
            e.stopPropagation();
            layui.event.call(eleNode, MOD_NAME, 'nodeDragBefore(' + that.config.id + ')', {
                current: {
                    node: eleNode,
                    data: o
                },
            ...
            if((options.isdrag && !o[options.isdrag]) || isStopDrag) return;
            that.layMain.css("user-select", "none");
            var dragNode = $('<div id="treeGridDragNode" class="treeGridDragNode"><i class="layui-icon iconfont '+((o.children&&o.children.length>0 || o[options.isfolder]) ? options.branch[1] : options.leaf)+'"></i><span class="node-txt">'+o.name+'</span></div>');
            dragNode.id = 'treeGridDragNode';
            var x = e.clientX ;
            var y = e.clientY;
            ...
            _DOC.on("mousemove", function(e) {
                ...
                if (time > 2) {
                    var xx = e.clientX  + 10;
                    var yy = e.clientY  + _DOC.scrollTop(); // 加上浏览器滚动高度
                    ...
                    if(target[0]){
                        ...
                        bodytr.removeClass('treegrid-row-append treegrid-row-top treegrid-row-bottom');
                        if(bodytr.parents('.layui-table').get(0).isEqualNode(target.parents('.layui-table').get(0))){
                            if (pageY > top + (bottom - top) / 2){
                                if (bottom - pageY < 5){
                                    target.addClass('treegrid-row-bottom');
                                } else {
                                    target.addClass('treegrid-row-append');
                                }
                            } else {
                                if (pageY - top < 5){
                                    target.addClass('treegrid-row-top');
                                } else {
                                    target.addClass('treegrid-row-append');
                                }
                            }
                ...
                _DOC.off("mousemove").off("mouseup");
                var target =e.target.nodeName == 'TR' ? $(e.target) : $(e.target).parents("tr"),
                targetIndex = parseInt(target.attr("data-index")),
                targetData = table.getDataList(that.config.id)[targetIndex],
                point = 'append';
                dragNode.remove();
                that.layMain.css("user-select", "auto");
                if(target[0]){
                    if (target.hasClass('treegrid-row-append')){
                        point = 'append';
                    } else {
                        point = target.hasClass('treegrid-row-top') ? 'insertBefore' : 'insertAfter';
                    }
                    target.removeClass('treegrid-row-append treegrid-row-top treegrid-row-bottom');
                }
                
                // 目标是否时最外层
                var isTargetOuterMost = $(e.target).get(0).isEqualNode(that.layMain.get(0))
                
                // 判断是否超出边界
                if (target.parents(that.layMain).length === 0 && !isTargetOuterMost) {
                    return;
                }
                // 判断初始与结束是否是同一个节点
                ...
                // 判断是否是父节点放到子节点
                if(targetData[that.config[TREE_UPID]] && targetData[that.config[TREE_UPID]] == o.id){
                    return;
                }
               
                layui.event.call(target, MOD_NAME, 'nodeDrag(' + that.config.id + ')', {
                    current: {
                        node: eleNode,
                        data: o
                    },
                ...
                table.moveRow(that.config.id, o, targetData, point);
                return false;
    ...
    Class.prototype.eventsinit = function(){
        ...
        that.layBody.on("click", "td", function(e) {
            var td = $(this)
              , field = td.data("field")
              , editType = td.data("edit")
              , elemCell = td.children(ELEM_CELL)
              , index = td.parents('tr').eq(0).data('index')
              , data = table.getDataList(that.key)[index]
              , options = that.config;
            layer.close(that.tipsIndex);
            if (!td.data("off") && editType) {
                 //显示编辑表单
                if(editType === 'select') { //选择框
                    var dropName=td.data('drop');
                    var rowsField=dl.ui.table.drop.findFieldObj(options.cols[0],field);
                    var o=dl.cache.code.get(rowsField.drop);
                    var html='';
                    ...
                    var select = $('<select class="'+ ELEM_EDIT +'" lay-ignore>' +
                        html+
                        '</select>');
                    td.find('.'+ELEM_EDIT)[0] || td.append(select);
                } else { //输入框
                    var input = $('<input class="layui-input '+ ELEM_EDIT +'">');
                    input[0].value = $.trim($(this).text());//  td.data('content') || elemCell.text();
            ...
            downfn.call(this, "hide")
        });
        var downName = "layui-table-grid-down"
          , downfn = function(hide) {
            var td = $(this)
              , cell = td.children(ELEM_CELL);
            if (hide){
                td.find(".layui-table-grid-down").remove();
            }else if (cell.prop("scrollWidth") > cell.outerWidth()) {
                if (cell.find("." + downName)[0]) return;
                td.append('<div class="' + downName + '"><i class="layui-icon layui-icon-down"></i></div>')
            }
        };
        that.layBody.on("click", "." + downName, function(e) {
            var down = $(this)
              , td = down.parent()
              , elemCell = td.children(ELEM_CELL);
        ...
        that.layBody.on('change','.'+ELEM_EDIT, function(){
			//huwei 20201107 添加旧数据做验证重复内容使用如果重复回复元数据，回调方法传input对象用于赋值旧数据
            var othis = $(this)
                ,value = this.value
