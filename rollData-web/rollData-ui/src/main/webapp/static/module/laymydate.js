...
;!function(window, document){
  ...
  */
  ...
  Class.prototype.mark = function(td, YMD){
    var that = this
    ,mark, name, options = that.config;
    lay.each(options.mark, function(key, obj){
      var keys = key.split('-');
      if((keys[0] == YMD[0] || keys[0] == 0) //每年的每月
      && (keys[1] == YMD[1] || keys[1] == 0) //每月的每日
      && keys[2] == YMD[2]){ //特定日
        mark = obj.title || YMD[2];
        name = obj.className
      }
    });
    mark && td.find('div').html('<span class="laydate-day-mark '+name+'">'+ mark +'</span>');
    
    return that;
  };

  // 公历重要月份和年份与自定义备注
  Class.prototype.yearMonthMark = function(type, YMD){
    var that = this
    ,mark, options = that.config;
    lay.each(options.mark, function(key, obj){
      var keys = key.split('-');
      if((type == "year" && keys[0] == YMD.year) || (type == "month" && keys[0] == YMD.year && keys[1] == YMD.month +1)){
        mark = "laydate-day-mark " + obj.className;
      }
    });
    return mark;
  };
  ...
  //是否当天，当月，当年
  Class.prototype.isCurr = function(type, YMD){
    var that = this
    ,ymd = that.systemDate();
    if(ymd && (type == "date" && ymd.year == YMD[0] && ymd.month + 1 == YMD[1] && ymd.date == YMD[2]) || (type == "month" && ymd.year == YMD.year && ymd.month == YMD.month) || (type == "year" && ymd.year == YMD.year)){
      return "laydate-ymd-curr";
    }else{
      return "";
    }
  };
  ...
  Class.prototype.calendar = function(value, index, type){
    ...
    lay.each(tds, function(index_, item){
      var YMD = [dateTime.year, dateTime.month], st = 0, curr = "";
      item = lay(item);
      ...
      YMD[1]++;
      YMD[2] = st + 1;
      curr = that.isCurr("date",YMD);
      item.attr('lay-ymd', YMD.join('-')).html('<div class="'+ curr +'">' + YMD[2] + '</div>');
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){
      ...
      lay.each(new Array(15), function(i){
        ...
        curr = that.isCurr("year", ymd);
        var title = yearNum + text,
        mark = that.yearMonthMark('year', ymd);
        if(mark){
          title = '<span class="'+mark+'">'+ title +'</span>'
        }
        li.innerHTML = '<div class="'+curr+'">' + title + '</div>';
        ul.appendChild(li);
    ...
    //生成月列表
      ...
      lay.each(new Array(12), function(i){
        ...
        curr = that.isCurr("month", ymd);
        var title = lang.month[i] + (isCN ? '月' : ''),
        mark = that.yearMonthMark('month', ymd);
        if(mark){
          title = '<span class="'+mark+'">'+ title +'</span>'
        }
        li.innerHTML = '<div class="'+curr+'">' + title + '</div>';
