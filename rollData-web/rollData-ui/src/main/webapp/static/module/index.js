...
layui.define(["layer", "element", "admin", "contextMenu", 'setter', 'wdUtil', 'pscrollbar'], function(s) {
	var d = layui.jquery;
	var r = layui.layer;
	var b = layui.element;
	var m = layui.admin;
	...
	var c;
	var h = {
		pageTabs: setter.pageTabs,
		cacheTab: setter.cacheTab,
		openTabCtxMenu: setter.openTabCtxMenu,
		maxTabNum: setter.maxTabNum,
		mTabList: [],
		...
		loadView: function(z) {
			...
			if (h.pageTabs) {
				...
				if (!v) {
					...
					b.tabAdd(i, {
						id: x,
						title: w ? '<span>'+w+'</span>' : '<span>无标题</span>',
						content: '<iframe lay-id="' + x + '" src="' + encodeURI(x) + '" frameborder="0" class="admin-iframe"></iframe>'
					});
					if (x != c) {
						h.mTabList.push(z)
					}
				...
				if (!u || u.length <= 0) {
					...
					y += '      <iframe lay-id="' + x + '" src="' + encodeURI(x) + '" frameborder="0" class="admin-iframe"></iframe>';
					y += "   </div>";
					d(j).html(y);
					if (x != c) {
						h.setTabTitle(w)
	...
	if (h.pageTabs && d(n).length <= 0) {
		...
		e += '               <dd ew-event="maxThisTabs" lay-unselect><a>全屏当前标签页</a></dd>';
	...
	b.on("nav(" + p + ")", function(x) {
		...
		if (u && u != "javascript:;" && u != 'null') {
			var v = w.text().replace(/(^\s*)|(\s*$)/g, "");
			if(w.attr('data-name') == '数据源管理' || w.attr('data-name') == '模型管理'){
				v = w.attr('data-name');
				var thistabid = '';
				d('.layui-layout-admin>.layui-body>.layui-tab .layui-tab-title>li>span').each(function(){
					if(d(this).text() == v){
						thistabid = d(this).parent().attr('lay-id');
						if(!d(this).parent().hasClass('layui-this')){
							d(this).parent().trigger('click');
						}
					}
				})
				if(thistabid){
				...
				var thisPaths = y.split("?");
				d('.layui-layout-admin>.layui-body>.layui-tab .layui-tab-title>li').each(function(){
					var that = d(this),
						thistabid = that.attr('lay-id');
					if(thistabid && thistabid != y && that.find("span").text() == v){
						var pathArr = thistabid.split("?");
						if(pathArr[0] == thisPaths[0]){
							var params = pathArr[1].split("&"),
								parames1 = thisPaths[1].split("&"),
								paramObj = {},
								paramObj1 = {};
							...
							if(parames1 && parames1.length){
								parames1.forEach(function(vv, ii){
									var str = vv.split("=");
									paramObj1[str[0]] = str[1]; 
								})
							}
							if(paramObj['relationName'] == v){
								if(paramObj['cid'] || paramObj['formId'] || paramObj['reportId']){
									if(paramObj['cid'] == paramObj1['cid'] || paramObj['formId'] == paramObj1['formId'] || paramObj['reportId'] == paramObj1['reportId']){
										d(this).attr("lay-id", y);
										return false;
									}
								}else{
									d(this).attr("lay-id", y);
	...
	b.on("tab(" + i + ")", function(w) {
		...
		var iframes = d(n + '>.layui-tab-content>.layui-tab-item:not(".layui-show") .admin-iframe');
		if (x) {
			x.style.height = "99%";
			x.scrollWidth;
			x.style.height = "100%";
			try {
				d(x).contents().find('body').removeClass('layui-hide-iframe');
			} catch (error) {
				
			}
		}
		iframes.each(function(ii, iframe){
			try {
				d(iframe).contents().find('body').addClass('layui-hide-iframe');
			} catch (error) {
	...
	d("body").off("click.navMore").on("click.navMore", "[nav-bind]", function() {
		...
		path = d(this).attr("data-path");
		if(!d(this).parent().hasClass("layui-this") || d(".layui-layout-admin").hasClass("admin-nav-mini")){
