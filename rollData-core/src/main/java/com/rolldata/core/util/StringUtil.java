...
public class StringUtil {
	 ...
	 * 标准代码开头标识
	 */
	public static final String startSymbol = "I";

    private static final Pattern p = Pattern.compile("\\s*|\t|\r|\n");
    public static final Pattern NBSP_COMPILE = Pattern.compile(" ");
	...
	public static String replaceBlank(String str) {
		if (str != null) {
			...
			str = m.replaceAll("");
            if (str.indexOf(' ') >= 0) {
                str = NBSP_COMPILE.matcher(str).replaceAll("");
            }
		}
		return str;
	}
...
//		}
		...
		/**
		 * 合并字符串 暂时支持两个 - 后期修改的时候 支持多个;
		 * @param 
		 * @return String
		 */
		public static String getMergeString(String s1, String s2) {
	     ...
	     * @returns String
	     */
	    public static String subBeginToEnd(String s) {
	        if (StringUtil.isNotEmpty(s)) {
	...
	/**
	 * 将元数据前补零,补后的总长度为指定的长度
	 * @param sourceDate 元数据
	 * @param formatLength 补0个数 "2"
	 * @return
	 */
	public static String frontCompWithZore(int sourceDate, String formatLength) {
	...
	public static boolean isBlank(CharSequence str) {
		...
		for (int i = 0; i < length; i++) {
			// 只要有一个非空字符即为非空字符串
			if (false == StringUtil.isBlankChar(str.charAt(i))) {
				return false;
			}
		}
	 ...
	 * @return
	 */
	public static String convertVariable(String str, Map<String, Object> paramsMap) {
		for (String key : paramsMap.keySet()) {
			try{
//			if (paramsMap.get(key) instanceof String) {
				str = str.replace("${" + key + "}", dealResultTypeToString(paramsMap.get(key)));
//			}
			}catch (Exception e){
				str = str.replace("${" + key + "}", " ");
			}
	 ...
	 * 判断结果类型返回字符串
	 * @param result
	 * @param isNull 是否区分null，不区分就是统一返回空串
	 * @return
	 */
	public static String dealResultTypeToString(Object result,boolean isNull) {
		if(isNull){
			if(result == null){
				return null;
			}
		}
		return dealResultTypeToString(result);
	}

	/**
	 * 判断结果类型返回字符串
	 * @param result 不区分null，统一返回字符串
	 * @return
	 */
	public static String dealResultTypeToString(Object result) {
		String returnStr = "";
		if(result instanceof String){
			returnStr = (String) result;
		}else if(result instanceof Short){
			Short tempResult = (Short)result;
			returnStr = tempResult.toString();
		}else if(result instanceof Integer){
			Integer tempResult = (Integer)result;
			returnStr = tempResult.toString();
		}else if(result instanceof BigInteger){
			BigInteger tempResult = (BigInteger)result;
			returnStr = String.valueOf(tempResult);
		}else if(result instanceof BigDecimal){
			BigDecimal tempResult = (BigDecimal)result;
			returnStr = String.valueOf(tempResult);
		}else if(result instanceof Long){
			Long tempResult = (Long)result;
			returnStr = tempResult.toString();
		}else if(result instanceof Boolean){
			Boolean tempResult = (Boolean)result;
			if(tempResult){
				returnStr = "1";
			}else{
				returnStr = "0";
			...
			Date tempResult = (Date)result;
			returnStr = tempResult.toString();
		}else if(result instanceof Time){
			Time tempResult = (Time)result;
			returnStr = tempResult.toString();
		}else if(result instanceof Timestamp){
			Timestamp tempResult = (Timestamp)result;
			returnStr = tempResult.toString();
		}else if(result instanceof Byte){
			Byte tempResult = (Byte)result;
			returnStr = tempResult.toString();
		}else if(result instanceof ArrayList){
			List tempResult = (ArrayList)result;
			if(tempResult.size()>0){
				returnStr = tempResult.get(0).toString();
			}
		}else if(result instanceof Clob){
			returnStr = clobToString((Clob)result);
		}
		return returnStr;
	}
	...
	public static String clobToString(Clob clob) {
		...
		try {
			...
			logger.error("将字CLOB转成STRING类型异常",e);
			reString = "";
		}
		return reString;
	}
	...
	/**
	 * 把集合,拼接成'value1','value2'
	 *
	 * @param values 集合
	 * @param symbol 包裹字段的符号(mysql=',sqlserver=")
	 * @return 拼接后的字符串
	 */
	public static String jointSql(Collection<String> values, String symbol) {
		...
		for (String value : values) {
			sb.append(symbol).append(value).append(symbol).append(",");
		}
